...
(() => {
  ...
  function renderExercises(meta) {
    const host = $("exercise-list");
    host.innerHTML = "";
    meta.exercises.forEach((ex, i) => {
      const label = document.createElement("label");
      label.className = "exercise-option";
      label.innerHTML = `
        <input type="radio" name="exercise" value="${ex.id}" ${i === 0 ? "checked" : ""}>
        <span class="exercise-name"></span>
        <span class="exercise-cal"></span>`;
      label.querySelector(".exercise-name").textContent = ex.name;
      label.querySelector(".exercise-cal").textContent = `Start: ${ex.calibration}`;
      label.querySelector("input").addEventListener("change", () => {
        state.exercise = ex.id;
        renderCalibrationNote(meta);
        refreshSubmit();
      });
      host.appendChild(label);
      if (i === 0) state.exercise = ex.id;
    });
  ...
  function renderCalibrationNote(meta) {
    const ex = state.exercises.find((e) => e.id === state.exercise);
    const pose = ex ? ex.calibration.toLowerCase() : "the start position";
    $("calibration-note").innerHTML = `
      <li>The first <strong>${meta.calibration_frames} detected frames</strong> are used to
          calibrate your body proportions — begin the clip holding the start
          position (${pose}) still.</li>
      <li>Film from the <strong>front</strong>. Every threshold was derived from
          front-view data, and side-on frames hide the left/right geometry the
          risk channels measure.</li>
      <li>Keep your whole body in frame; frames with low-confidence hips or knees
          are skipped by the rep counter.</li>`;
  }
  ...
  function refreshSubmit() {
    $("analyse-btn").disabled = !(state.file && state.exercise);
  }
  ...
  async function submitJob(event) {
    ...
    if (!state.file || !state.exercise) return;

    const body = new FormData();
    body.append("video", state.file);
    body.append("exercise", state.exercise);
    body.append("mode", "stream");
  ...
  // ── Module-1 signal control ──────────────────────────────────────────────
  function renderSignalBar() {
    const bar = $("signal-bar");
    const host = $("signal-buttons");
    if (!state.signals.length) { bar.hidden = true; return; }
    bar.hidden = state.mode !== "live";
    host.innerHTML = state.signals.map((s) =>
      `<button type="button" class="signal-btn ${s.kind}" data-label="${s.label}">
         ${escapeHtml(s.name)}</button>`).join("");
    host.querySelectorAll(".signal-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        sendControl({ type: "signal", label: btn.dataset.label });
      });
    });
    highlightSignal(state.activeSignal);
  }
